  ...
  <div id="app">
     ...
     <van-row gutter="20" class="row-1">
      ...
      <van-col span="2" class="cols">
        <van-icon name="qr" class="classfic" @click="路由跳转('/about')"/>
      </van-col>
    </van-row>
    ...
    <!-- tabBar -->
      <van-tabbar v-model="tabarActive">
        <van-tabbar-item  icon="wap-home" @click="路由跳转('/')">首页</van-tabbar-item>
        <van-tabbar-item icon="pending-evaluate" dot @click="路由跳转('/community')">社区</van-tabbar-item>
        <van-tabbar-item icon="shopping-cart" info="5" v-infos="shop_info" @click="路由跳转('/shoppingCart')">购物车</van-tabbar-item>
        <van-tabbar-item icon="contact" info="2" v-infos="my_info" @click="路由跳转('/me')">我的</van-tabbar-item>
      </van-tabbar>
    </div>
</template>
